...
**Reason:** User asked for "mais histórias de interatividade para dar mais vida às atividades". Every task was the same interaction with different text - walk to a console, answer a popup - and the room themes were decorative only.
**Trade-off:** Emergencies are deliberately **positive-stakes**: failing one never kills anyone and never ends the match. For a game aimed at children a countdown that can wipe the crew turns a shared moment into dread. The blackout is punishing enough while it runs; the leak simply does not heal you.
**Impact:** The blackout blinds **crewmates only** - impostors keep full vision, applied identically to bots server-side and the human client-side. That asymmetry is what makes cutting the lights a hunt rather than a nuisance. Bots take a randomised 4-16s to notice an alarm: without that delay a bot standing near the panel resolved a blackout in about one second and the player never saw the lights go out. A deferred event (meeting running) retries in 12s rather than skipping a whole 70s interval, or one meeting silently swallows the next few minutes.

### AD-011: Crewmates get one random spell per match (2026-08-02)

**Decision:** Every crewmate is dealt one of three abilities at match start and may cast it once (key Q): **Clarão** (blinds everyone who can see you for 2s and hastes you for 3s), **Radar** (reveals all players on the minimap for 4s), **Embaralhar** (teleports every living player to a random room). Impostors get none.
**Reason:** User asked for a crewmate ability, once per turn, then for all the suggested ones with a random deal per player per match. The design pressure came from AD-010: making a kill take three hits gave the victim time to react but nothing to react *with* - Clarão is that missing counterplay.
**Trade-off:** Casting proves you are a crewmate to anyone watching, since impostors have no spell. Kept deliberately: it is a real cost paid for a real benefit, and it gives meetings something concrete to argue about. Radar is the one sanctioned exception to the limited-vision rule - it is scoped to the *map* only (3D avatars stay hidden) and radar contacts are drawn dimmer than people genuinely in sight, so the map never implies eyes-on.
**Impact:** `shared/spellPool.js` is pure data + a pure picker (unit-tested for reachability of every spell). `MatchState` gains `spellByPlayer` and `spellsSpent`. Clarão blinds bots through the same line-of-sight predicate everything else uses, and **a blinded bot records no sightings at all** - so casting beside a murder destroys the witness evidence, which is what makes it interesting rather than merely strong. Crewmate bots spend their charge when dropped to 1 health. Embaralhar is the only spell the server executes itself, because only it knows where the bots are.

### AD-010: Kills take three hits, and impostor count is configurable (2026-08-02)

**Decision:** Attacking damages rather than kills - `MAX_HEALTH = 3` hits, at a much shorter range (bot attack range 2.5 → 1.4). The victim's screen flashes red and shows a health readout; every player's current health is drawn above their head. A match can have more than one impostor, chosen by the host, defaulting to 1.
**Reason:** User: *"As mortes estão acontecendo muito facil, os npcs só chegam perto e já matam o jogador"* - a single touch at 2.5 units meant deaths felt arbitrary and unavoidable.
**Trade-off:** `PLAYER_HURT` is broadcast to everyone so the over-head health can update, which is the first thing that deliberately leaks combat state past the limited-vision rule (AD from the vision work). Accepted: seeing that someone is being hurt is information the game *should* surface. Also, multiple impostors tighten the roster maths - with N impostors parity is N-vs-N, so the count is clamped to `floor((total-1)/2)`, i.e. 2 at the standard 6 players.
**Impact:** `match.impostorId` became `impostorIds: Set` - a migration touching `getRole`, `checkWinCondition`, `tallyVotes`, `doPlayerLeft`, the `gameOver` payload, and the client's game-over screen. `health: Map` lives in `MatchState` beside `alive`, but `recordDeath` stays the single transition out of `alive` so every existing death path (ejection, disconnect) and the win conditions are untouched. Bots gained a short inter-hit cooldown (1.2s) separate from the long post-kill stand-down, and keep pursuing between hits. Verified live: two impostors, health sequences 2→1→0 per victim, match concluding normally.

### AD-009: The world pauses while a player is answering a task question (2026-08-02)

**Decision:** While any human has a task quiz open, the server stops simulating bots entirely (`botRunner.setPaused`), driven by a new `busy` protocol message. Kill and vent cooldowns are pushed forward by the paused duration so a pause cannot be used to bank a free kill.
**Reason:** The user reported: *"Quando estou respondendo os quiz o jogo está continuando e não me esperando"*. With AD-008's educational tasks, a question can take a child a minute; being killed part-way through a sum punishes precisely the behaviour the tasks exist to encourage, and standing frozen at a console is when you are most vulnerable.
**Trade-off:** In a match with several humans, one player opening a quiz freezes the bots for everyone. Other humans keep moving, so the game does not fully stop - and the target case is a child playing solo with bots, where this is exactly right. Accepted rather than engineering per-player time dilation.
**Impact:** `shared/protocol.js` gains `BUSY`; `server/index.js` keeps a `busyPlayers` set and syncs the pause on join/leave/match start; `botRunner` gains `setPaused`. Verified live: bot movement updates went 180 → 0 → 180 across pause and resume.
...
### L-017: "The agent's position is legal" is not the same as "the route is passable" (2026-08-02)

**Context:** After L-016 fixed bots pathing through walls, the user reported bots *still* going through walls, adding the decisive detail: *"Eu não consigo ir atrás deles"* - they could not follow.
**Problem:** My verification had asserted that every sampled bot position lay on floor (2059, then 4252 samples, zero failures). That was true and useless: it tested the *floor*, never the *walls*. The real defect was in the geometry, not the pathfinder. Each straight corridor segment built its two side walls across its full run - including across the mouth of the perpendicular segment it turns into at a bend. Bots, which have no collision at all, walked straight through those stubs; the player's capsule hit them. Measuring the thing the user actually described - sampling corridor centrelines against *wall* boxes - found **39 blocked points**, all at bends. The user's "I can't follow them" was the part of the report my test could not have produced, and it was the part that identified the bug.
**Solution:** Corridor walls are now built in a second pass, after all floor exists, and a wall piece is only emitted where there is no floor immediately on the other side of it. One rule covers bends, corridor-to-corridor junctions, and corridor mouths at rooms, instead of enumerating those cases. Re-measured with a *player-sized* body (0.35 radius, not a point): 0 blocked samples, 0 floor gaps, and the octree got faster (386ms/178MB from 554ms/240MB) because there is less wall geometry.
**Prevents:** When verifying that something is traversable, test the thing that blocks traversal. A test built from the same mental model as the code will confirm that model rather than challenge it - I checked "is the bot on the floor" because the previous bug had been missing floor. Also: an agent that ignores collision cannot validate a level's collision, so "the bot got there" says nothing about whether a player can. Sample with the player's actual body radius, against the actual blocking geometry.

### L-018: A win condition written for exactly one of something breaks quietly when there are two (2026-08-02)

**Context:** Making the impostor count configurable meant `match.impostorId` (a single id) became `impostorIds` (a set).
**Problem:** The dangerous part was not the rename but the two places that encoded "one" as arithmetic rather than as a lookup. `checkWinCondition`'s crew-win branch was `!alive.has(match.impostorId)` - with two impostors that fires the moment *either* is ejected, ending the match with one still alive. And `const livingCrewCount = match.alive.size - 1` silently hardcoded the impostor count in a subtraction. Neither would have thrown; both fail open, and `getRole` reading an undefined `impostorId` would have returned `'crewmate'` for everybody - the worst possible silent failure in a hidden-role game.
**Solution:** Enumerated every `impostorId` read across the server and client before editing any of them (six in gameState, three in gameActions, two in the client), then wrote the multi-impostor win-condition tests *first* and watched them fail. The crew-win branch became "no living impostor" and parity became `livingCrew <= livingImpostors`.
**Prevents:** When a field goes from one to many, grep for the *arithmetic* that assumed one, not just for the identifier. `size - 1`, `=== theOneId`, and `find(...)` all encode the old cardinality without mentioning it, and none of them fail loudly. Write the "now there are two" test before the migration so the assumption has something to break against.
